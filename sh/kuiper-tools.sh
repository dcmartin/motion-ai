...
kuiper.rule.create()
{
  ...
  curl -sSL ${KUIPER_HOST:-localhost}:${KUIPER_PORT:-9081}/rules -X POST -d '{"id":"'${name}'","sql":"select * from '${stream}'","actions":[{"mqtt":{"server":"tcp://'${MQTT_USERNAME:-username}:${MQTT_PASSWORD:-password}@${MQTT_HOST:-127.0.0.1}:${MQTT_PORT:-1883}'","topic":"'${topic}'"}}]}'
}
